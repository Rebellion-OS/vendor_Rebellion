...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to RebellionOS Gerrit Review.
...
- repopick:        Utility to fetch changes from RebellionOS Gerrit.
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/rebellion/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch rebellion_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Rebellion-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.rebellion.device | grep -q "$REBELLION_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $REBELLION_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/RebellionOS/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop ro.rebellion.device | grep -q "$REBELLION_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.rebellion.device | grep -q "$REBELLION_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/rebellion/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
